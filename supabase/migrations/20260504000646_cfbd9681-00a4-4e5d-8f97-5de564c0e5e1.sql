...
BEGIN
  ...
  -- 1) Delete invoice_items tied to student_fees of this fee structure
  WITH del AS (
  ...
  -- 2) Delete payments for invoices that become empty (only this school)
  ...
  -- 3) Delete student_fees for this fee structure (scoped via fee_structure_id)
  WITH del_sf AS (
...
BEGIN
  ...
  -- student_fees has no school_id; scope via fee_structures
  WITH d AS (
    DELETE FROM public.student_fees sf
    USING public.fee_structures fs
    WHERE sf.fee_structure_id = fs.id AND fs.school_id = p_school_id
    RETURNING sf.id
