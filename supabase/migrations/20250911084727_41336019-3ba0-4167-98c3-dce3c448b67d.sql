-- Separate sensitive student information into protected tables
...
-- Medical information policies (only authorized medical personnel, principals, admins)
...
USING (EXISTS (
  ...
  AND profiles.role = ANY (ARRAY['admin'::user_role, 'principal'::user_role, 'nurse'::user_role])
))
WITH CHECK (EXISTS (
  SELECT 1 FROM profiles 
  WHERE profiles.user_id = auth.uid() 
...
-- Emergency contact policies (safety personnel, teachers for emergencies, principals, admins)
...
-- Remove sensitive columns from students table
