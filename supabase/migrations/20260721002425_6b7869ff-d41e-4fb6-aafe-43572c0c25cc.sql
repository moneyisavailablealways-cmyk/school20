...
BEGIN
  ...
  IF v_app.application_type = 'learner' THEN
    ...
    IF NULLIF(v_app.parent_name, '') IS NOT NULL THEN
      ...
      SELECT p.id INTO v_parent_details_id
      FROM public.parents p
      WHERE p.school_id = v_app.school_id
        AND v_app.parent_national_id IS NOT NULL
        AND p.national_id = v_app.parent_national_id
      LIMIT 1;
      ...
      IF v_parent_details_id IS NULL THEN
        ...
        ON CONFLICT (national_id) DO UPDATE SET address = EXCLUDED.address
...
BEGIN
  RETURN jsonb_build_object(
    'success', false,
    'error', 'Admissions must be approved through the secure auth-first approval service. Please refresh and try again.'
  );
END;
$$;

GRANT EXECUTE ON FUNCTION public.approve_admission_application(uuid, uuid, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.approve_admission_application(uuid, uuid, uuid) TO service_role;